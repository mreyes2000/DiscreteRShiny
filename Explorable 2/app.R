...
sidebar <- dashboardSidebar(
  ...
  btn_sidebar("btnr2", "Apply r<sup>2</sup>"),
  btn_sidebar("btnr3", "Apply r<sup>3</sup>"),
  btn_sidebar("btns", "Apply s"),
  btn_sidebar("btnrs", "Apply rs"),
  btn_sidebar("btnr2s", "Apply r<sup>2</sup>s"),
  btn_sidebar("btnr3s", "Apply r<sup>3</sup>s")
)
...
server <- function(session, input, output) {
  ...
  observeEvent(input$btnr2, {
    config <<- D4.apply("r<sup>2</sup>",config)
    output$square <- renderPlot(D4.showSquare(config))
  })
  
  observeEvent(input$btnr3, {
    config <<- D4.apply("r<sup>3</sup>",config)
    output$square <- renderPlot(D4.showSquare(config))
  })
  ...
  observeEvent(input$btnr2s, {
    config <<- D4.apply("r<sup>2</sup>s",config)
    output$square <- renderPlot(D4.showSquare(config))
  })
  
  observeEvent(input$btnr3s, {
    config <<- D4.apply("r<sup>3</sup>s",config)
    output$square <- renderPlot(D4.showSquare(config))
  })
}
